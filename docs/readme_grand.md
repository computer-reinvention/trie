...
- **M14** ✓ — type-aware reference precision: a pluggable LSP resolver supplements the tree-sitter pass with method/member-dispatch edges (we benchmarked tree-sitter vs LSP and chose LSP over SCIP for language coverage). Seven languages ship — Python, TypeScript/TSX, JavaScript/JSX, Go, Rust, C, Lua — each pairing a tree-sitter backend with an optional language server (basedpyright/pyright, typescript-language-server, gopls, rust-analyzer, clangd, lua-language-server), configurable via `[resolver]` in `trie.toml`
- **v0.2** — latency tuning for cold language servers, more languages (Java, C#, Ruby, Swift as new resolver specs), vector-over-triefacts retrieval, `trie watch` daemon, rename detection in reconcile
