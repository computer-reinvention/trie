...
def parse_hist_mass(token: str | None) -> tuple[float, float]:
    """Parse a `hist_mass` sentinel token `<value>@<ts>` into (mass, ts).

    Returns (0.0, 0.0) for None/malformed tokens so legacy sentinels (no
    hist_mass) and corrupt values both read as "no historical mass yet".
    """
    if not token:
        return (0.0, 0.0)
    try:
        value_str, _, ts_str = token.partition("@")
        return (float(value_str), float(ts_str) if ts_str else 0.0)
    except (ValueError, TypeError):
        return (0.0, 0.0)


def format_hist_mass(mass: float, ts: float) -> str:
    """Serialize (mass, ts) into the `<value>@<ts>` sentinel token.

    Mass is quantized to one decimal so quiet attention changes don't churn the
    triefact diff; ts is an integer second (sub-second precision is irrelevant
    on a 21-day decay horizon).
    """
    return f"{mass:.1f}@{int(ts)}"
...
class Section:
    ...
    historical_mass: float = 0.0  # AGM cross-session cognitive-importance signal.
    historical_mass_ts: float = 0.0  # unix ts the mass was last stamped (for decay).
    # Persisted as `hist_mass=<value>@<ts>` in the sentinel — the triefact is the
    # source of truth (reproducible from version control), the graph DB column is
    # rebuildable cache. Updated only when a symbol's triefact is regenerated by
    # `trie sync`. NOT decayed live mass: it counts recurrence across
    # investigations (see trie.attention_store + the sync-time fold).
...
class TriefactFile:
    ...
    def parse(cls, text: str) -> TriefactFile:
        ...
        for open_match in SECTION_OPEN_RE.finditer(rest):
            ...
            if body.endswith("\n"):
                body = body[:-1]
            hist_mass, hist_mass_ts = parse_hist_mass(open_match.group("hist_mass"))
            chunks.append(
                Section(
                    ...
                    historical_mass=hist_mass,
                    historical_mass_ts=hist_mass_ts,
                )
            )
    ...
    def upsert_section(
        ...
        *,
        ...
        """
        # Preserve any existing AGM historical mass: regenerating a section's
        # prose must not wipe the cross-session importance signal. The sync-time
        # fold updates mass explicitly via set_section_historical_mass.
        existing = self.get_section(qualified_name)
        new = Section(
            ...
            role=role,
            historical_mass=existing.historical_mass if existing else 0.0,
            historical_mass_ts=existing.historical_mass_ts if existing else 0.0,
        )
    ...
    def set_section_historical_mass(self, qualified_name: str, mass: float, ts: float) -> bool:
        """Stamp the AGM historical mass on an existing section, preserving body,
        role, and all other fields. Returns True if the section existed.

        This is the durable half of the sync-time historical-mass fold: it writes
        the cross-session cognitive-importance signal into the sentinel without
        touching prose or fingerprints, so the fold produces a minimal diff (only
        hist_mass= changes). Mass is quantized at render time so quiet attention
        changes don't churn the file.
        """
        for i, c in enumerate(self.chunks):
            if isinstance(c, Section) and c.qualified_name == qualified_name:
                self.chunks[i] = replace(c, historical_mass=mass, historical_mass_ts=ts)
                return True
        return False
    ...
    def render(self) -> str:
        ...
        for c in self.chunks:
            if isinstance(c, Prose):
                ...
                # hist_mass last (after role) so legacy sentinels stay byte-identical
                # until next regenerated. Omitted when zero so untouched symbols
                # don't gain a noisy field.
                if c.historical_mass > 0.0:
                    fields.append(
                        f"hist_mass={format_hist_mass(c.historical_mass, c.historical_mass_ts)}"
                    )
