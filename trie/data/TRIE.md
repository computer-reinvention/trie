...
- Calling the agent's built-in `grep` (whatever its prefix in the
  current harness) **is the same as calling `«grep»`** — the wrapper
...
- For opencode, two extra tools, `trie_read` and `trie_trace`, appear
  alongside the harness's normal tools. They expose `«read»` and
  `«trace»` under names that don't collide with any built-in.
