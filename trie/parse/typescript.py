...
class TypeScriptBackend:
    ...
    def lsp_backends(self):
        ...
        # tsc emits human-readable diagnostics by default; parsed by the "tsc"
        # output_format added to the edit pipeline. Pretty/color disabled for
        # stable parsing.
        return [
            LspBackend(
                command="tsc",
                check_args=["--noEmit", "--pretty", "false"],
                output_format="tsc",
                exit_ok_codes=[0],
            )
        ]
